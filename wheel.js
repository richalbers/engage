...
const PIN_AND_POINTER_COLOR = "lightgray";  //must be a shade of gray or pointer deflection code doesn't work
...
Wheel.prototype.drawDivider = function(ctx, angleRad, x, y) {
	ctx.fillStyle = PIN_AND_POINTER_COLOR; 
	ctx.strokeStyle = PIN_AND_POINTER_COLOR;
...
Wheel.prototype.drawPointer = function(ctx) {
	...
	// Determine if pointer needs to deflect.  If pixel on edge of wheel where pointer will
	// be is a shade of gray (i.e. it's a pin) then pointer needs to deflect
	var imgData=ctx.getImageData(this.width/2 + this.outerRadius-1, this.height/2 ,1,1);
	if ((imgData.data[0] == imgData.data[1]) && (imgData.data[1] == imgData.data[2] )) //some shade of gray
	{
		yDeflection=3;
	}	
